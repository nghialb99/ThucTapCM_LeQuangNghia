...
{
    public partial class UserCrtReportImportInventory : UserControl
    {
        public UserCrtReportImportInventory()
        {
            InitializeComponent();
        }
        ...
        {
            dgvListInvoice.DataSource = ImportInventory_DAO.Instance.GetListImportGoods(dateTimeFrom.Value, dateTimeTo.Value);
        ...
        private void btExportToExcel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                app.Visible = true;
                //dua du lieu
                worksheet.Cells[1, 4] = "BẢNG THỐNG KÊ PHIẾU NHẬP HÀNG TỒN";
                worksheet.Cells[2, 3] = "";
                worksheet.Cells[3, 1] = "STT";
                worksheet.Cells[3, 2] = "Mã phiếu nhập tồn";
        ...
        private void SetColorRowWhenBillStatusIsDelete()
        {
            for (int i = 0; i < dgvListInvoice.Rows.Count; i++)
            {
                if (dgvListInvoice.Rows[i].Cells[5].Value.ToString() == "Xóa bỏ")
                {
